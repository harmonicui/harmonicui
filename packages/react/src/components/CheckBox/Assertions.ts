...
export type AssertionsConfigurationOptions = Partial<{
  ...
  states: Array<'invalid' | 'optional' | 'disabled' | 'indeterminate'>
...
export class CheckBoxAssertions {
  ...
  private readonly NUMBER_OF_INPUT_ASSERTIONS = 7
  ...
  private assertInputHasProperIndeterminateState(): void {
    if (this.states.indeterminate) {
      expect(this.getInput()).toHaveAttribute('indeterminate.prop', 'true')
    } else {
      expect(this.getInput()).toHaveAttribute('indeterminate.prop', 'false')
    }
  }
  ...
  private assertHelperTextHasProperVisibility(): void {
    if (this.states.invalid || this.states.indeterminate) {
