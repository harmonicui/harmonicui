...
import { provideCheckBoxInputContext, CheckBoxInputContract } from '../../contexts'

function getCheckBox () {
  return container.querySelector('input')
}

function renderTemplate (template: string, contextValue: Partial<CheckBoxInputContract> = {}) {
...
describe('indeterminate attribute', () => {
  ...
  test('indeterminate attribute should not be overridable by user', () => {
    renderTemplate(
      '<CheckBoxInput indeterminate.prop="true" />',
      { indeterminate: false },
    )
...
describe('value attribute', () => {
  ...
  test('value attribute should not be overridable by user', () => {
    renderTemplate(
      '<CheckBoxInput value = "propsValue" />',
      { value: 'contextValue' },
    )
  ...
  test('consumes and updates the modelValue provided by the CheckBoxInputContext', async () => {
    const modelValue = ref<CheckBoxInputContract['modelValue']>([])
    const setModelValue: CheckBoxInputContract['setModelValue'] = (newValue) => {
      if (modelValue.value instanceof Array) {
        modelValue.value.includes(newValue)
          ? modelValue.value.splice(modelValue.value.indexOf(newValue), 1)
          : modelValue.value.push(newValue)
      } else {
        modelValue.value === newValue ? modelValue.value = '' : modelValue.value = newValue
      }
    }
    const { getByTestId } = renderTemplate(
      '<CheckBoxInput value="inputValue" data-testid="input" />',
      {
    ...
    expect(modelValue.value).toContain('inputValue')

    await fireEvent.change(getByTestId('input'))

    expect(modelValue.value).not.toContain('inputValue')
  })
})
...
describe('aria-* attributes', () => {
  ...
  test('consumes aria-errormessage attribute from CheckBoxInputContext', () => {
    ...
    expect(getCheckBox()).toHaveAttribute('aria-errormessage', 'errormessage-id')
  })
