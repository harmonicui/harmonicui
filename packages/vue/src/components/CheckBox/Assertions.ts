...
export type AssertionsConfigurationOptions = Partial<{
  ...
  states: Array<'invalid' | 'optional' | 'disabled' | 'checked' | 'indeterminate'>
  ignoreAssertions: Array<'Input' | 'Label' | 'HelperText' | 'ErrorMessage'>
}>

export class CheckBoxAssertions {
  private IDs = {
    input: 'HUI-CheckBoxInput-',
    label: 'HUI-CheckBoxLabel-',
    helperText: 'HUI-CheckBoxHelperText-',
    errorMessage: 'HUI-CheckBoxErrorMessage-',
  }

  private readonly NUMBER_OF_INPUT_ASSERTIONS = 8
  ...
  private get totalAssertions () {
    return Object.values(this.assertionsToRun).reduce((total, next) => total + next)
  }
  ...
  private runInputAssertions (): void {
    if (this.assertionsToRun.Input) {
      ...
      this.assertInputHasProperCheckedState()
      this.assertInputHasProperIndeterminateState()
  ...
  private assertInputHasProperId (): void {
    expect(this.getInput()).toHaveAttribute(
      'id', expect.stringMatching(new RegExp('^' + this.IDs.input)),
    )
  }

  private assertInputHasProperDescription (): void {
    if (this.assertionsToRun.HelperText && !this.states.invalid) {
      return expect(this.getInput()).toHaveAttribute('aria-describedby', this.getHelperText().id)
    }
  ...
  private assertInputHasProperErrorMessage (): void {
    if (this.assertionsToRun.ErrorMessage && this.states.invalid) {
      return expect(this.getInput()).toHaveAttribute('aria-errormessage', this.getErrorMessage().id)
    }
  ...
  private assertInputHasProperCheckedState (): void {
    if (this.states.checked) {
      return expect(this.getInput()).toBeChecked()
    }

    return expect(this.getInput()).not.toBeChecked()
  }

  private assertInputHasProperIndeterminateState (): void {
    if (this.states.indeterminate) {
      return expect(this.getInput()).toHaveAttribute('indeterminate.prop', 'true')
    }

    return expect(this.getInput()).toHaveAttribute('indeterminate.prop', 'false')
  }
  ...
  private assertLabelHasProperId (): void {
    expect(this.getLabel()).toHaveAttribute(
      'id', expect.stringMatching(new RegExp('^' + this.IDs.label)),
    )
  }
  ...
  private assertLabelMissedDataIsAttributeWhenNoState (): void {
    if (!this.states.invalid && !this.states.optional && !this.states.disabled) {
  ...
  private assertLabelHasProperDataIsAttributeWhenDisabled (): void {
    if (this.states.disabled) {
      return expect(this.getLabel()).toHaveAttribute('data-is', expect.stringContaining('disabled'))
    }

    expect(this.getLabel()).not.toHaveAttribute('data-is', expect.stringContaining('disabled'))
  }

  private assertLabelHasProperDataIsAttributeWhenOptional (): void {
    if (this.states.optional) {
      return expect(this.getLabel()).toHaveAttribute('data-is', expect.stringContaining('optional'))
    }

    expect(this.getLabel()).not.toHaveAttribute('data-is', expect.stringContaining('optional'))
  }

  private assertLabelHasProperDataIsAttributeWhenInvalid (): void {
    if (this.states.invalid) {
      return expect(this.getLabel()).toHaveAttribute('data-is', expect.stringContaining('invalid'))
    }

    expect(this.getLabel()).not.toHaveAttribute('data-is', expect.stringContaining('invalid'))
  }

  private assertHelperTextHasProperId (): void {
    expect(this.getHelperText()).toHaveAttribute(
      'id', expect.stringMatching(new RegExp('^' + this.IDs.helperText)),
    )
  }
  ...
  private assertErrorMessageHasProperId (): void {
    expect(this.getErrorMessage()).toHaveAttribute(
      'id', expect.stringMatching(new RegExp('^' + this.IDs.errorMessage)),
    )
  }
